 ...
 * @param index 
 */
void lireDataSet(float dataset[NBLINE][NBCOL], int classes[NBLINE][NB_CLASSES],int d,int nb_col,int nb_classes){
...
void KNN2(LweSample *predictedClass[NB_CLASSES],float dataset[NBLINE][NBCOL],int labels[NBLINE][NB_CLASSES],LweSample *M[NBCOL],const LweParams *params, TFheGateBootstrappingCloudKeySet *bootk,int d,int m,int k,int nb_col,int nb_classes){
    /**************************************************************************************/
    /************************* Constrction de delta matrices ******************************/
    if (VERBOSE) cout << "Construction la matrice des delta values"<<endl;
    ...
    LweSample *classVector[NBLINE][NB_CLASSES];
    #pragma omp parallel for collapse(2)
    for(int i=0;i<d;i++){
        for(int j=0;j<nb_classes;j++)
        classVector[i][j]=new_LweSample(params);
    }
clock_t begin = clock();
    deltaValuesMatrix(deltaValues,dataset,M, params,bootk->bkFFT,m,d,nb_col);
    ...
    for(int i=0;i<d;i++){
        for(int j=0;j<nb_classes;j++){
            lweNoiselessTrivial(temp_result, AndConst, bootk->bkFFT->in_out_params);
            lweNoiselessTrivial(label,modSwitchToTorus32(labels[i][j]==0?-1:1,4*m-4),params);
            lweAddTo(temp_result, label, bootk->bkFFT->in_out_params);
            lweAddTo(temp_result, deltaVector[i], bootk->bkFFT->in_out_params);
            lweCopy(classVector[i][j],temp_result,bootk->bkFFT->in_out_params);
            tfhe_bootstrap_woKS_FFT(classVector[i][j],bootk->bkFFT, modSwitchToTorus32(1, 4*m-4),temp_result);
            lweNoiselessTrivial(temp,modSwitchToTorus32(1,4*m-4),params);
            lweAddTo(classVector[i][j], temp, bootk->bkFFT->in_out_params);
        }
    ...
    for(int j=0;j<nb_classes;j++){
        lweClear(predictedClass[j],params);
        for(int i=0;i<d;i++){
            lweAddTo(predictedClass[j],classVector[i][j],params);
        }   
    }
...
int main(int argc, char** argv){
    ...
    int32_t nb_classes = 2;
    char cmd[512] ="python3 entrainmentKNNIris.py ";
    char d_car[6];
    sprintf(d_car, "%d ", d);
    strcat(cmd, d_car );
    sprintf(d_car, "%d ", k);
...
TFheGateBootstrappingCloudKeySet *bootk=new TFheGateBootstrappingCloudKeySet(paramsGate, bk, bs_key);
    ...
    int classes[NBLINE][NB_CLASSES];
    lireDataSet(dataset,classes,d,nb_col,nb_classes);
...
/**************************** KNN Function ******************************************/
    ...
    begin =clock();
    LweSample *predictedClass[NB_CLASSES];
    for(int i= 0;i<nb_classes;i++){
        predictedClass[i]=new_LweSample(paramsLwe);
    }
   // KNN(deltaVector,classVector,dataset,classes,MEncrypted,paramsLwe,bootk,d,m,k,nb_col);
   double debut=omp_get_wtime();
    KNN2(predictedClass,dataset,classes,MEncrypted,paramsLwe,bootk,d,m,k,nb_col,nb_classes);
double fin=omp_get_wtime();
    ...
    Torus32 predicted;
    for(int i=0;i<nb_classes;i++){
        predicted=lweSymDecrypt(predictedClass[i],key_lwe,4*m-4);
        cout << "Class predite "<< i <<": "<<t32tod(predicted)<<endl;
    }
